...
**Status as of 2026-08-11:** Main tab (server-owner content) is fully translated, including all Guides sub-pages. The **API tab** now exists in German with the `General` group (Developer Overview, 1 page) plus 15 products fully translated: Jaksam Inventory (63 pages), Drugs Creator (40 — completed, see 1l below), Doors Creator (20), Missions Creator (15), Blips Creator (2), Farming Creator (8), Shops Creator (18), Robberies Creator (12), Vehicles Keys (36), Dealerships Creator (13), Races Creator (7), Trackers Creator (8), Easy Allowlist (12), Billing UI (13), Luxury Clothes Theft (2) — 271 API pages total. **Only Jobs Creator (66 pages) remains untranslated in the entire API tab.** Once that's done, the API tab is 100% bilingual.
...
### 1l. ~~Drugs Creator~~ — DONE as of 2026-08-11, completed across 2 sessions/branches (`de-locale-drugs-client` then `de-locale-drugs-server`)

Drugs Creator has the most nested structure of any product (7 subgroups: 2 client, 5 server) — 40 pages total, deliberately split into 2 chunks due to session usage-limit constraints, same reasoning as the earlier Jaksam Inventory API split.

**Client (16 pages):** index + 6 leaf pages (replace-default-notifications, replace-disable-default-progress-bar, replace-default-mugshot-notifications, replace-default-help-notifications, replace-default-police-alert, manually-start-drugs-effects) + nested NPC Selling group (index + 5 pages) + nested Territories group (index + 2 pages).

**Server + modules.md (24 pages):** `modules.md` (top-level advanced-integration page — note: the *English* source text has a copy-paste bug, it says "Modules are an easy way for **Jobs Creator**..." instead of Drugs Creator; translated faithfully as-is per the no-new-info rule, not "fixed", same precedent as the Shops Creator `police-alerted.md` issue), Server index, then 5 nested subgroups: Fields (index + 1), Harvestable Items (index + 1), Laboratories (index + 1), Sellings (index + 6: sold-to-pusher, sold-in-boat, sold-in-plane, sold-to-npc, sold-to-narcos, sold-by-hired-dealer), Territories (index + 4: ownership-changed, give-territory-points, remove-territory-points, reset-territory-points), Hired Dealers (index only, 0 sub-pages in English too), Drug Factories (index only, 0 sub-pages in English too), plus 2 standalone leaf pages (police-alerted, pocket-crafting-finished).

Inserted into `docs.json` as a `"Drugs Creator"` group between `Inventory` and `Doors Creator` — its correct English-nav position — now containing `modules.md` + full `Client` + full `Server` subgroups, matching English exactly. The German home page's "Entwickler-Referenz" card was updated to `/de/drugs-creator/client` now that the whole product is done. Also fixed one more pre-existing stale-English-link bug found in passing (same recurring pattern as Trackers Creator / Vehicles Keys): `de/drugs-creator/faq.md` linked to the English `/drugs-creator/client/manually-start-drugs-effects` despite the German translation existing at that path.

### 1m. ~~General / Developer Overview~~ — DONE as of 2026-08-11 (branch `de-locale-dev-overview`)

Translated `developers/overview.mdx` → `de/developers/overview.mdx` (1 page, `tag: "New"` kept). Added the `"General"` group to the German API tab in `docs.json`, first position (matching English order: General, Inventory, ...). The page's "Jump to a product's API reference" CardGroup links to every product's client/server root — **each href was updated to `/de/<product>/client` (or `/server` for Easy Allowlist) for every fully-translated product**; the **Jobs Creator** card intentionally still points at the English `/jobs-creator/client` since no German translation exists there at all. When Jobs Creator eventually gets translated, update that one href too (grep `href="/jobs-creator` in `de/developers/overview.mdx`).

### 2. API tab (developer reference) — 66 pages remaining: Jobs Creator only

Jobs Creator is the last untranslated product in the entire API tab. This is everything currently under the English `"API"` tab in `docs.json` that isn't German yet:

- Jobs Creator's `Client`/`Server` export & event pages (all of it, including nested subgroups — check the English `docs.json` structure first, this product has more nesting than most: Notifications, Actions, Duty, Buyable Vehicles, Garage Owned, Temporary Garage under Client; Armory, Boss, Buyable Vehicles, Crafting Table, Duty, Harvest, Market, Shop, Process, Search/Steal Action under Server)
- `jobs-creator/modules.md` (top-level advanced-integration page)

**Recommendation:** This is the last remaining product and the largest (66 pages, more deeply nested than Drugs Creator or Vehicles Keys) — treat it as its own dedicated multi-session effort, same way the original restructure was phased (audit → plan → execute). Given the size, consider splitting Client and Server into separate sessions like Drugs Creator, or splitting by sub-area. Once Jobs Creator is done, **the API tab is 100% bilingual and this file's job is finished** — at that point, update the memory file `project_jaksam_docs_migration.md` to reflect full completion and this continuation doc can be archived/deleted. **Remember to insert the new group at its correct English-nav position** (between `Inventory` and `Drugs Creator`, i.e. first among the products, matching English order: General, Inventory, Jobs Creator, Drugs Creator, ...) — see the note at the top of this file.
...
As of 2026-08-11 this should print, in this exact order: `General, Inventory, Drugs Creator, Doors Creator, Missions Creator, Blips Creator, Farming Creator, Shops Creator, Robberies Creator, Vehicles Keys, Dealerships Creator, Races Creator, Trackers Creator, Easy Allowlist, Billing UI, Luxury Clothes Theft`. `Drugs Creator` is now fully complete (modules.md + Client + Server). Only `Jobs Creator` is missing from this list entirely — once added, it belongs between `Inventory` and `Drugs Creator`. If the order doesn't match, something changed since this note was written — re-derive the current state before continuing.
