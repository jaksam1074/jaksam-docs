...
<AccordionGroup>
  <Accordion title="getStaticItemsList">
    ...
    **Returns:** `items` (table) — The list of items, key is the item name, value is item information (label, maxStack, weight, stackable, description, rarity, type, etc.)
  ...
  <Accordion title="getStaticItem">
    ...
    **Returns:** `item` (table) — The item information, if the item is not found, it will return nil
  ...
  <Accordion title="getItemLabel">
    ...
    **Returns:** `label` (string | nil) — The label (display name) of the item, or nil if the item is not found
  ...
  <Accordion title="getItemImagePath">
