...
<Steps>
  <Step title="Restart the script">
    ...
    <Frame caption="in-game F8 restart example">
      ![in-game F8 restart example](/files/8T47Gzl2nXMMl31RbAYa)
    </Frame>

    <Frame caption="server console restart example">
      ![server console restart example](/files/SSQl3xGfww9x0SPXcCrY)
    </Frame>
  </Step>
...
<Frame caption="Solution: see Error parsing script below">
  ![Escrow error example](/files/xshHg32DJhBTUGv7YHwz)
</Frame>

<Frame caption="Solution: see Error parsing script below">
  ![Escrow error example](/files/OWcZXwGMM67pEyfymeb7)
</Frame>

<Frame caption="Solution: see Failed to verify protected resource below">
  ![Escrow error example](/files/aZRk3irzR5ZWnlrsllcZ)
</Frame>

<Frame caption="Solution: see You lack the required entitlement below">
  ![Escrow error example](/files/1evHGngvFfcVtguEXhCN)
</Frame>
...
<AccordionGroup>
  <Accordion title="Error parsing script ... <\1>">
    ...
    **Example**
    ...
    <Frame>
      ![version command example](/files/aPB7A5nf96u479kSvsnE)
    </Frame>
  ...
  <Accordion title="Failed to verify protected resource">
    ...
    <Steps>
      <Step title="Re-download the script">
        Download the script again from [FiveM keymaster](https://keymaster.fivem.net/).
      </Step>
  ...
  <Accordion title="You lack the required entitlement">
    ...
    - The server key is generated in [FiveM keymaster](https://keymaster.fivem.net/)
    ...
    **Example:**
    ...
    <Steps>
      ...
      <Step title="Open FiveM keymaster">
        Go to [FiveM keymaster](https://keymaster.fivem.net/).
      </Step>
      <Step title="Verify script ownership">
        Verify if the account you are logged in with on [FiveM keymaster](https://keymaster.fivem.net/) owns the script(s) you are trying to use. If the script is not there, it means you used a different account on Tebex, or you have transferred the script to another account.
        ...
        **Example with** [**Jobs Creator script**](https://forum.cfx.re/t/jobs-creator-4-6-esx-in-game-job-creation-menu-without-server-restart/2667762)
        ...
        <Frame>
          ![Jobs Creator ownership example](/files/jVwB2VLNOnX8Tzi1cuzY)
        </Frame>
      </Step>
      <Step title="Verify the server key owner">
        If the account you are using on [FiveM keymaster](https://keymaster.fivem.net/) owns the script(s), verify if the **server key** your server is using is generated by the same account.

        <Frame>
          ![Server key owner example](/files/31Dyg6QSMMDKnmrE2jfX)
        </Frame>
      </Step>
    </Steps>
    ...
    **Example screenshot:**
    ...
    <Frame>
      ![ZAP Hosting license key example](/files/qfoS13Gg8cqsd7A2L938)
    </Frame>
  </Accordion>
